...
async function autoExtract(sendProgress) {
  ...
  try {
    for (let guard = 0; guard < 120; guard++) {
      ...
      if (!seen.has(key)) {
        ...
        if (record?.questions?.length) {
          ...
          // Keep questionNumber backward-compatible as the global sequence.
          q.questionNumber = all.length + 1;
          all.push(q);
          seen.add(key);
          sendProgress?.({ done: all.length, total: 100, questionNumber: q.globalQuestionNumber, section: sectionName, sectionQuestionNumber: localQn });
        }
      }
      ...
      if (sectionCount && localQn === sectionCount) {
        ...
        if (!moved) throw new Error(`Reached ${sectionName} Q${localQn}, but could not move to the next section.`);
        sectionIndex++;
      ...
      if (!nextButtonExists()) {
        const moved = await moveToNextSection(localQn);
        if (!moved) throw new Error(`Stopped at ${sectionName} Q${localQn}: Next button not found and could not move to the next section.`);
        sectionIndex++;
        previousSectionName = currentSectionName();
        await wait(500);
        continue;
    ...
    return { extractedAt: new Date().toISOString(), pageTitle: document.title, url: location.href, count: all.length, questions: all };
